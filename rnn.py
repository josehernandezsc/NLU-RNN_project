...
class RNN(object):
	...
	def predict(self, x):
		...
		for t in range(len(x)):
			if t==0:
				net_in=self.V[:,x[t]]+np.matmul(self.U,s[-1,:])
			else:
				net_in=self.V[:,x[t]]+np.matmul(self.U,s[t-1,:])
	...
	def acc_deltas(self, x, d, y, s):
		...
		for t in reversed(range(len(x))):
			d_vec=np.zeros(len(y[t]))
			d_vec[d[t]]+=1
			delta_out=np.multiply((d_vec-y[t]),np.ones(len(y[t])))

			f_der=np.multiply(s[t],1-s[t])
			delta_in=np.multiply(np.matmul(np.transpose(self.W),delta_out),f_der)
	...
	def acc_deltas_np(self, x, d, y, s):
		...
		d_vec=np.zeros(len(y[-1,:]))
		d_vec[d[0]]+=1
		
		delta_out=np.multiply((d_vec-y[-1,:]),np.ones(len(y[-1,:])))
		
		f_der=np.multiply(s[-2],1-s[-2])
		delta_in=np.multiply(np.matmul(np.transpose(self.W),delta_out),f_der)

		self.deltaW+=np.outer(delta_out,s[-2])

		
		x_vec=make_onehot(x[len(x)-1],self.vocab_size)
		self.deltaV+=np.outer(delta_in,x_vec)

		self.deltaU+=np.outer(delta_in,s[len(x)-2])
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		for t in reversed(range(len(x))):
			...
			for tau in range(steps+1):
				x_vec=np.zeros(self.vocab_size)
				x_vec[x[t-tau]]+=1
				f_der=np.multiply(s[t-tau],1-s[t-tau])
				if tau==0:
					delta_in=np.multiply(np.matmul(np.transpose(self.W),delta_out),f_der)
				else:
					delta_in=np.multiply(np.matmul(np.transpose(self.U),delta_in),f_der)
				self.deltaV+=np.outer(delta_in,x_vec)
	...
	def acc_deltas_bptt_np(self, x, d, y, s, steps):
		...
		t=len(x)-1
		for tau in range(steps+1):
			#Check if I need to break loop upon lookback limit
			if(len(x)<tau):
				break
	...
	def compute_loss(self, x, d):
		...
		d_matrix=np.zeros((len(x),y.shape[1]))
		for i,k in enumerate(d):
			d_matrix[i,k]+=1
		loss=-np.sum(np.multiply(np.log(y),d_matrix))
	...
	def compute_loss_np(self, x, d):
		...
		loss = 0.
		y=self.predict(x)[0][-1,:]
		
		d_matrix=np.zeros((y.shape[0]))
		d_matrix[d[0]]+=1
		loss=-np.sum(np.multiply(np.log(y),d_matrix))
	...
	def train(self, X, D, X_dev, D_dev, epochs=10, learning_rate=0.5, anneal=5, back_steps=0, batch_size=100, min_change=0.0001, log=True):
		...
		loss_sum_train = sum([len(d) for d in D])
		...
		for epoch in range(epochs):
			...
			###################### UNCOMMENT ONLY FOR QUESTION 2B #############################
			loss_train = sum([loss_function(X[i], D[i]) for i in range(len(X))])/loss_sum_train
			
			if log:
				stdout.write("\tepoch done in %.02f seconds" % (time.time() - t0))
				stdout.write("\tnew training loss: {0}".format(loss_train))
...
if __name__ == "__main__":
	...
	if mode == "train-lm":
		...
		'''
		train_size = 25000 #original value=1000
		dev_size = 1000
		vocab_size = 2000
		...
		model.train(X_train, D_train, X_dev, D_dev, epochs=10, learning_rate=lr, anneal=5, back_steps=lookback, batch_size=32, min_change=0.0001, log=True)

		#compute loss for test
		run_loss=model.compute_mean_loss(X_test,D_test)
		...
		####### SAVE PARAMETERS
		with open("results/rnn.U.npy","wb") as f:
			np.save(f,model.U)
		with open("results/rnn.V.npy","wb") as f:
			np.save(f,model.V)
		with open("results/rnn.W.npy","wb") as f:
			np.save(f,model.W)
	...
	if mode == "train-np":
		...
		model.train_np(X_train, D_train, X_dev, D_dev, epochs=10, learning_rate=lr, anneal=5, back_steps=lookback, batch_size=32, min_change=0.0001, log=True)
		
		acc=sum([model.compute_acc_np(X_test[i], D_test[i]) for i in range(len(X_test))]) / len(X_test)
